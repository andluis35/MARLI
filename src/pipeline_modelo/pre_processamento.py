...
def isolar_variaveis(df):
    ...
    colunas_para_remover = [
        ...
        'HistoricoVitoriasEmpresaVencedora',
        'PercentualDesconto',
        'SemPublicidadePNCP',
        "ModalidadeDeRisco",
        'ScoreDeRisco'
    ...
    # Mantém apenas: 'Ente', 'Ano', 'Mes', 'Modalidade', 'Objeto', 'ValorEstimado' e 'Risco'.
...
def codificar_categoricas(df):
    ...
    df = df.copy()
    le = LabelEncoder()
    colunas_categoricas = ['Ente', 'Modalidade']
...
def tratar_nulos(X_train, X_test):
    ...
    if 'ValorEstimado' in X_train.columns:
        mediana_valor = X_train['ValorEstimado'].median()
        X_train['ValorEstimado'] = X_train['ValorEstimado'].fillna(mediana_valor)
        X_test['ValorEstimado'] = X_test['ValorEstimado'].fillna(mediana_valor)
...
def normalizar_numericas(X_train, X_test):
    ...
    scaler.fit(X_train[['ValorEstimado']])

    X_train[['ValorEstimado']] = scaler.transform(
        X_train[['ValorEstimado']]
    )

    X_test[['ValorEstimado']] = scaler.transform(
        X_test[['ValorEstimado']]
    )
